...
function menu_tab() {
    $menu_title = esc_html__('Get Started with Royal Shop Options', 'royal-shop');
    add_theme_page( esc_html__( 'Royal Shop', 'royal-shop' ), $menu_title, 'edit_theme_options', 'royal_shop_thunk_started',array($this,'tab_page'));
...
if ($hook === 'appearance_page_royal_shop_thunk_started'  ) {
wp_enqueue_style( 'thunk-started-css', get_template_directory_uri() . '/lib/th-option/assets/css/started.css' );
wp_enqueue_script('royal-shop-admin-load', get_template_directory_uri() . '/lib/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);

$data = apply_filters(
                    'th_option_localize_vars',
                    array(
                        'oneClickDemo' =>esc_url( admin_url( 'themes.php?page=z-companion-sites' )),

                        )
                );
    wp_localize_script( 'royal-shop-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('ThemeHunk','royal-shop'),
    'theme_brand_url' => esc_url($theme_data->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'royal-shop'), esc_html($theme_data->get( 'Name' ),'royal-shop'), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html($theme_data->get( 'Name' ).' is a Free WooCommerce theme for creating clean and professional shopping stores.', 'royal-shop' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','royal-shop'),
       'post_content' => '',
       'post_status' => 'publish',
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'royal-shop' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'royal-shop' );
...
$boj = new Royal_Shop_theme_option(); ?>
